package test_shjchan::test_shjchanClient;
...
test_shjchan::test_shjchanClient
...
sub new
{
    ...
    my $self = {
	client => test_shjchan::test_shjchanClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a test_shjchan.FilterContigsParams
$output is a test_shjchan.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a test_shjchan.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a test_shjchan.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "test_shjchan.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for test_shjchan::test_shjchanClient\n";
    }
    if ($sMajor == 0) {
        warn "test_shjchan::test_shjchanClient version is $svr_version. API subject to change.\n";
    }
}
...
package test_shjchan::test_shjchanClient::RpcClient;
